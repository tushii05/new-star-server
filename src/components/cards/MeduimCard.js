...
export default function MeduimCard({
  ...
  const langInfo = i18n.locales.find((lang) => lang.id === post?.lang_id);
  const short_form = langInfo?.short_form;
  return (
    <div className={className ?? "col-lg-4 col-md-4"}>
      <div className="news-card card-icon-m1">
        <CustomLink href={`/${post?.title_slug}`} lang={short_form ?? locale}>
          <Image
            width={380.656}
            height={246.359}
            src={checkImage(
            ...
            className="img-fluid"
            sizes="(max-width : 992px) 380.656px, 100%"
            loading="lazy"
            placeholder="blur"
        ...
        <BookmarkPost articleId={post?.id} saved={saved} />
        <div className="row pt-2 cat-row">
          <div className="col-6">
            <p className="cat-name mb-0">
              <CustomLink
                lang={short_form ?? locale}
                href={"/" + (post?.category?.name_slug ?? categoryName)}
              >
        ...
        <p className="news-card-desc mb-2 text-truncate-2">{post?.summary}</p>
        <div className="row news-date-time mt-2">
          ...
          <div className="col-6 d-flex justify-content-end">
            <p className="mb-0">
              <span className="date me-3">{formatDate(post?.created_at)}</span>
              <span className="time">
