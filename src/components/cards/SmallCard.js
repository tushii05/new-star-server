...
export default function SmallCard({ post, className, locale, categoryName }) {
  return (
    <div className="item">
      <div className={"news-card mb-0 pe-3 " + className}>
        ...
        <div className="row news-date-time mt-2">
          ...
          <div className="col-7 d-flex justify-content-end">
            <p className="mb-0">
              <span className="date me-2">{formatDate(post?.created_at)}</span>
              <span className="time">{timeAgo(post?.created_at)}</span>
            </p>
          </div>
        </div>
        {checkPostType(post?.post_type, 50, 50)}
      </div>
    </div>
  );
}
