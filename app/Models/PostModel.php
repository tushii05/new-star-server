...
class PostModel extends BaseModel
{
    ...
    public function getMostViewedPosts($limit)
    {
     ...
     public function get_posts_grouped_by_month() 
    {
        $sql="SELECT DATE_FORMAT(created_at, '%Y-%m') AS post_month FROM posts GROUP BY YEAR(created_at), MONTH(created_at) ORDER BY created_at DESC";
...
public function getPostsByMonthYear($month, $year)
{
    ...
    // Retrieve posts for the given date range
    $sql = "SELECT * 
            FROM posts 
    ...
    // Prepare and execute the query
            print_r($this->db->query($sql)->getResult());
    ...
    return $this->db->query($sql)->getResult();  ;  // or $this->db->query($sql)->getResult();
}
    ...
    public function getPostPreview($slug)
    {
        ...
        return $this->builder->where('posts.title_slug', cleanSlug($slug))->groupStart()->where('posts.status', 0)->orWhere('posts.is_scheduled', 1)->orWhere('posts.visibility', 0)->groupEnd()->get()->getRow();
    }
    ...
    public function getRSSPosts($userId, $categoryId, $categories, $limit)
    {
        ...
        @unlink(WRITEPATH . 'cache/' . $key);
        cache()->save($key, $posts, RSS_CACHE_REFRESH_TIME);
        return $posts;
    }
    ...
    {
        $posts = cache('scheduled_posts');
        $isUpdated = false;
        if (!empty($posts)) {
            $date = date('Y-m-d H:i:s');
            if (!empty($posts)) {
                foreach ($posts as $post) {
                    if ($post->status == 1 && strtotime($post->created_at) <= strtotime($date)) {
        ...
        if ($isUpdated) {
            @unlink(WRITEPATH . 'cache/scheduled_posts');
    ...
    public function setScheduledPostsCache()
    {
        ...
        if (!empty($posts)) {
            cache()->save('scheduled_posts', $posts, 7200);
        }
    }
