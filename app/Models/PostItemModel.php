...
class PostItemModel extends BaseModel
{
    ...
    public function getPostListItems($postId, $postType)
    {
        ...
        if ($postType == 'table_of_contents') {
            return $this->builderSortedListItems->select("post_sorted_list_items.*, 'table_of_contents' AS item_post_type")->where('post_id', cleanNumber($postId))->orderBy('item_order')->get()->getResult();
        }
    ...
    public function getPostListItem($itemId, $postType)
    {
        ...
        if ($postType == 'table_of_contents') {
            return $this->builderSortedListItems->select("post_sorted_list_items.*, 'table_of_contents' AS item_post_type")->where('id', cleanNumber($itemId))->get()->getRow();
        }
