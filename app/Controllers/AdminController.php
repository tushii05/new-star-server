...
use App\Models\WebStoryModel;
use App\Models\WebSubStoryModel;
...
class AdminController extends BaseAdminController
{
    ...
    public function addEpapers()
    {
        // checkAdmin();
        $data['title'] = trans("epaper");
        $data['users'] = $this->authModel->getActiveUsers();

        if ($this->request->getPost('epaper_group') != NULL) {
            ...
            $validation->setRules([
                'epaper_group' => 'required',
                'epaper_date' => 'required',
                'image' => 'uploaded[image]|mime_in[image,application/pdf]|max_size[image,51200]'
            ]);
            ...
            if ($file->isValid() && !$file->hasMoved()) {
                ...
                // Optionally, check if the file already exists and handle conflicts
                $filePath = 'uploads/epaper/' . $fileName;
                if (file_exists($filePath)) {
                    // Add a unique suffix to avoid overwriting
                    $fileName = pathinfo($fileName, PATHINFO_FILENAME) . '_' . time() . '.' . $file->getExtension();
                }

                // Move the file to the uploads directory with the modified name
                $file->move('uploads/epaper', $fileName);

                // Prepare data for the database
                $date = $this->request->getPost('epaper_date');
                $epaperDate = date('d-m-Y', strtotime($date));
                $insertData = [
                    'epaper_group' => $this->request->getPost('epaper_group'),
                    'epaper_date' => $epaperDate,
                    'image' => 'uploads/epaper/' . $fileName
                ];

                // Insert data using model
                $epaperModel = new EpaperModel();
                $epaperModel->insert_epaper($insertData);

                // Redirect or show success message
                return redirect()->back()->with('success', 'E-paper uploaded successfully.');
            } else {
            ...
            // Fetch all epapers from the table
            $data['epaper'] = $epaperModel->getAllEpapers();

            return view('admin/add-epaper', $data);
            echo view('admin/includes/_footer');
        }
    }
    ...
    {
        //checkAdmin();
         $webStoryModel = new WebStoryModel();
        $data['title'] = trans("epaper");
       $data['users'] = $this->authModel->getActiveUsers();
         $data['webStories'] = $webStoryModel->findAll();
    ...
    public function addWebStory()
    {
        ...
        if ($this->request->getMethod() === 'post') {
            $validationRules = [
                'title' => 'required',
                'slug' => 'required|url_title',
                'content' => 'required',
                'cover_image' => 'uploaded[cover_image]|max_size[cover_image,2048]|mime_in[cover_image,image/jpg,image/jpeg,image/png]',
                'meta_title' => 'required',
            ...
            if ($file->isValid() && !$file->hasMoved()) {
                $newName = $file->getRandomName(); // Generate a new name for the file
                $file->move('uploads/web_stories', $newName); // Move file to the desired location

                // $stringgg = $this->request->getPost('slug');
                // $slug = str_replace(" ", "-", $stringgg);

                $stringgg = $this->request->getPost('slug');

                $slug = strtolower(trim($stringgg)); 
                ...
                $data = [
                    ...
                    'cover_image' => 'uploads/web_stories/' . $newName, // Adjust path as necessary
                    'meta_title' => $this->request->getPost('meta_title'),
                    'meta_description' => $this->request->getPost('meta_description'),
                ];

                // Insert data into the database
                $webStoryModel->insert($data);
                
                // Fetch all web stories to display
           

                // Redirect or show a success message
                return redirect()->back()->with('success', 'Web Story added successfully!');
            } else {
                return redirect()->back()->withInput()->with('error', 'Cover image upload failed.');
            }
        }

    }
    public function editWebStory()
    {
          //checkAdmin();
          $webStoryModel = new WebStoryModel();
          $data['title'] = trans("epaper");
...
public function addSubWeb($id)
    {
        //checkAdmin();
        $webStoryModel = new WebSubStoryModel();
        $data['title'] = trans("title");
        $data['users'] = $this->authModel->getActiveUsers();
        // $data['webStories'] = $webStoryModel->findAll();
        $data['webStories'] = $webStoryModel->where('web_id', $id)->findAll();
        
        $data['webstory_id'] = $id;
        // Load the test_page view
        echo view('admin/includes/_header', $data);
        return view('admin/add_subweb'); // Make sure app/Views/test_page.php exists
         echo view('admin/includes/_footer');
    }

    public function addWebSubStory()
    {
        // print_r('Mantu');
        //         die();
        // Load necessary models
        ...
        if ($this->request->getMethod() === 'post') {
            ...
            if ($file->isValid() && !$file->hasMoved()) {
                ...
                $data = [
                    ...
                    'content' => $this->request->getPost('content'),
                    'cover_image' => 'uploads/web_stories/' . $newName, // Adjust path as necessary
                    'web_id' => $this->request->getPost('web_id')
                ];
