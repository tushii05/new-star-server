...
class HomeController extends BaseController
{
    ...
    public function index()
    {
        ...
        $data['latestPosts'] = getLatestPosts($this->activeLang->id, POST_NUM_LOAD_MORE + 2);
    ...
    public function posts()
    {
        ...
        $cacheKey = 'posts_page_' . $data['pager']->currentPage;
        $data['posts'] = getCachedData($cacheKey);
        if (empty($data['posts'])) {
            $data['posts'] = $this->postModel->getPostsPaginated($this->postsPerPage, $data['pager']->offset);
...
public function loadPostsByMonthYear($monthYear)
{
    list($month, $year) = explode('-', $monthYear);
    
    // Validate and sanitize input
    if (!checkdate($month, 1, $year)) {
        return $this->error404(); // Handle invalid dates
    }
    ...
    // Load and return the HTML for posts
    return($data); // Ensure _post_list.php is the view for the posts list
    ...
    private function category($category, $isParent = true)
    {
        ...
        if ($isParent && $category->parent_id != 0) {
            ...
            $data['pager'] = paginate($this->postsPerPage, $numRows);
            $cacheKey = 'posts_category_' . $category->id . '_page' . $data['pager']->currentPage;
            $data['posts'] = getCachedData($cacheKey);
            if (empty($data['posts'])) {
                $data['posts'] = $this->postModel->getPostsByCategoryPaginated($category->id, $this->categories, $this->postsPerPage, $data['pager']->offset);
                setCacheData($cacheKey, $data['posts']);
            }
    ...
    public function tag($tagSlug)
    {
        ...
        $data['pager'] = paginate($this->postsPerPage, $numRows);
        $data['posts'] = $this->postModel->getTagPostsPaginated($tagSlug, $this->postsPerPage, $data['pager']->offset);
    ...
    public function search()
    {
        ...
        $data['pager'] = paginate($this->postsPerPage, $numRows);
        $data['posts'] = $this->postModel->getSearchPostsPaginated($q, $data['searchInContent'], $this->postsPerPage, $data['pager']->offset);
