...
class PostController extends BaseAdminController
{
    ...
    public function addPost()
    {
        checkPermission('add_post');
        $type = inputGet('type');
        if ($type != 'article' && $type != 'gallery' && $type != 'sorted_list' && $type != 'table_of_contents' && $type != 'video' && $type != 'audio' && $type != 'trivia_quiz' && $type != 'personality_quiz' && $type != 'poll' && $type != 'recipe') {
            $type = 'article';
        }
    ...
    public function addPostPost()
    {
        ...
        $val->setRule('category_id', trans("category"), 'required');
         $val->setRule('post_image_id', trans("post_image_id"), 'required');
    ...
    */
    ...
    public function deletePostAdditionalImage()
    {
        ...
        public function get_posts_by_month()
    {
    $this->load->model('Post_model');
    $data['months'] = $this->Post_model->get_posts_grouped_by_month();
