...
class CategoryController extends BaseAdminController
{
    ...
    public function editCategory($id)
    {
        ...
        $data['subParentCategories'] = $this->categoryModel->getSubCategoriesByParentId($data['category']->parent_id);
        $settingsModel = new SettingsModel();
        $data['widgets'] = $settingsModel->getWidgets();
    ...
    public function getCityCategories()
    {
        $parentId = inputPost('parent_id');
        if (!empty($parentId)) {
            $subCategories = $this->categoryModel->getCityCategoriesByParentId($parentId);
            foreach ($subCategories as $item) {
                echo '<option value="' . $item->id . '">' . $item->name . '</option>';
            }
        }
