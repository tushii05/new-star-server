...
if (!function_exists('getLatestPosts')) {
    function getLatestPosts($langId, $limit)
    {
        ...
        $posts = getCachedDataByLang($key, $langId);
        if (empty($posts)) {
            $model = new  \App\Models\PostModel();
            $posts = $model->getLatestPosts($langId, $limit);
            setCacheDataByLang($key, $posts, $langId);
        }
        return $posts;
    }
}
...
if (!function_exists('get_posts_grouped_by_month')) {
    function get_posts_grouped_by_month()
    {
        
        
            $model = new  \App\Models\PostModel();
            $posts = $model->get_posts_grouped_by_month();         
